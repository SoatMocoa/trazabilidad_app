...
def display_invoice_entry_form(user_role):
    ...
    with st.form(key=f"invoice_entry_form_{st.session_state.form_key}", clear_on_submit=False):
        ...
        numero_factura = st.text_input("Numero de Factura:",
                                       value=st.session_state.current_invoice_data[1] if st.session_state.current_invoice_data and not st.session_state.refacturar_mode else "",
                                       disabled=st.session_state.refacturar_mode)
        fecha_generacion = st.text_input("Fecha de Generacion (YYYY-MM-DD o DD/MM/YYYY):",
                                         value=st.session_state.current_invoice_data[4] if st.session_state.current_invoice_data and not st.session_state.refacturar_mode else "",
                                         disabled=st.session_state.refacturar_mode)
        
        area_servicio_default_index = 0
        if st.session_state.current_invoice_data and not st.session_state.refacturar_mode:
            try:
                area_servicio_default_index = AREA_SERVICIO_OPCIONES.index(st.session_state.current_invoice_data[2]) + 1
            except ValueError:
                area_servicio_default_index = 0

        area_servicio = st.selectbox("Area de Servicio:", options=[""] + AREA_SERVICIO_OPCIONES,
                                     index=area_servicio_default_index,
                                     disabled=st.session_state.refacturar_mode)
        ...
        if user_role == 'legalizador' and st.session_state.current_invoice_data and st.session_state.current_invoice_data[14] == 'Devuelta por Auditor':
            if st.form_submit_button("Marcar como Corregida"):
                ...
                # st.rerun() is called inside marcar_como_corregida_action via cancelar_edicion_action
        ...
        if submitted:
            if st.session_state.refacturar_mode:
                guardar_factura_reemplazo_action(st.session_state.editing_factura_id, new_numero_factura, fecha_reemplazo_factura,
    ...
    with st.form("bulk_load_form"):
        ...
        if bulk_submitted and uploaded_file is not None:
            ...
            for index, row in df.iterrows():
                ...
                try:
                    fecha_generacion_csv_obj = datetime.strptime(fecha_str_csv, '%Y-%m-%d').date()
                except ValueError:
                    try:
                        fecha_generacion_csv_obj = datetime.strptime(fecha_str_csv, '%d/%m/%Y').date()
                    except ValueError:
                ...
                factura_id = db_ops.insertar_factura_bulk(
                    numero_factura_csv, area_servicio_bulk, facturador_bulk,
                    fecha_generacion_db, eps_bulk, fecha_hora_entrega_db
                )

                if factura_id:
                    if area_servicio_bulk == "SOAT":
                        db_ops.insertar_detalles_soat_bulk(factura_id, fecha_generacion_db)
                    inserted_count += 1
...
def display_invoice_table(user_role):
    ...
    for factura in facturas_raw:
        ...
        try:
            fecha_generacion_obj = datetime.strptime(fecha_generacion_base_str, '%Y-%m-%d').date()
        except ValueError:
            fecha_generacion_obj = hoy_obj
        ...
        processed_facturas.append({
            ...
            "EPS": eps_nombre,
            "Numero de Factura": display_numero_factura_col,
            "Numero Reemplazo Factura": display_numero_reemplazo_col,
            "Fecha Generacion": display_fecha_generacion_actual_col,
            "Fecha Reemplazo Factura": display_fecha_reemplazo_display,
            "Fecha de Entrega": fecha_hora_entrega,
            "Dias Restantes": display_dias_restantes,
            "Estado": display_estado_for_tree,
            "Estado Auditoria": estado_auditoria_db,
            "Tipo de Error": tipo_error_db,
            "Observacion Auditor": observacion_auditor_db,
    ...
    selected_invoice_id = st.number_input(
        "ID de Factura para Accion:",
        min_value=0,
        step=1,
        key=f"selected_invoice_id_input_{st.session_state.selected_invoice_input_key}" # Dynamic key
    )
        ...
        if factura_data_for_action:
            ...
            if user_role == 'auditor':
                ...
                if st.session_state.current_invoice_data and st.session_state.current_invoice_data[14]:
                    try:
                        estado_auditoria_default_index = ESTADO_AUDITORIA_OPCIONES.index(st.session_state.current_invoice_data[14])
                    except ValueError:
                        estado_auditoria_default_index = 0
                ...
                if st.session_state.current_invoice_data and st.session_state.current_invoice_data[16]:
                    try:
                        tipo_error_default_index = TIPO_ERROR_OPCIONES.index(st.session_state.current_invoice_data[16])
                    except ValueError:
                        tipo_error_default_index = 0
                ...
                # Display confirmation dialog if 'confirm_delete_id' is set to the current selected ID
                if 'confirm_delete_id' in st.session_state and st.session_state.confirm_delete_id == selected_invoice_id:
                    ...
                    with col_cancel_del:
                        if st.button("Cancelar", key="cancel_delete_button_modal"):
                            ...
                            st.session_state.confirm_delete_id = None # Clear confirmation state
                            st.rerun() # Rerun to clear message
        else:
...
def guardar_factura_action(facturador, eps, numero_factura, fecha_generacion_str, area_servicio):
    ...
    try:
        fecha_generacion_obj = datetime.strptime(fecha_generacion_str, '%Y-%m-%d').date()
    except ValueError:
        try:
            fecha_generacion_obj = datetime.strptime(fecha_generacion_str, '%d/%m/%Y').date()
        except ValueError:
...
def cargar_factura_para_edicion_action(factura_id):
    ...
    if factura_data:
        ...
        st.error("No se pudo cargar la factura para edicion.")
...
def auditar_factura_action(factura_id, nuevo_estado_auditoria, observacion, tipo_error):
    ...
    success = db_ops.actualizar_estado_auditoria_factura(
        factura_id, nuevo_estado_auditoria, observacion_to_save, tipo_error_to_save
    )
...
def guardar_factura_reemplazo_action(old_factura_id, new_numero_factura, fecha_reemplazo_factura_str,
    ...
    if not fecha_reemplazo_factura_str:
        ...
        return
    try:
        fecha_reemplazo_factura_obj = datetime.strptime(fecha_reemplazo_factura_str, '%Y-%m-%d').date()
    except ValueError:
        try:
            fecha_reemplazo_factura_obj = datetime.strptime(fecha_reemplazo_factura_str, '%d/%m/%Y').date()
        except ValueError:
...
def marcar_como_corregida_action(factura_id, observacion_actual, tipo_error_actual):
    ...
    success = db_ops.actualizar_estado_auditoria_factura(
        factura_id, "Corregida por Legalizador", observacion_actual, tipo_error_actual
    )
    if success:
...
def export_df_to_csv(df):
    ...
    st.download_button(
        label="Descargar CSV",
        data=csv,
        file_name="facturas_trazabilidad.csv",
        mime="text/csv",
    )

# --- Control de flujo principal de la aplicación Streamlit ---
